...
import type { PublicSitePayload, SiteConfig, ThemePresetId } from "../types";
import { applyPaletteCssVars, getPaletteTuple } from "../lib/themePresets";
...
export function PublicShell() {
  ...
  useEffect(() => {
    ...
    if (!cfg) return;
    document.title = cfg.siteName?.trim() || "技术生存服务器";
  ...
  useEffect(() => {
    const cfg = payload?.config as SiteConfig | undefined;
    if (!cfg) return;
    const tuple = getPaletteTuple(cfg.colorPreset as ThemePresetId | undefined, cfg.customPalette);
    applyPaletteCssVars(document.documentElement, tuple);
  }, [payload]);
        ...
        <header className="topbar-shell">
          <div className="container topbar">
            <Link to="/" className="brand brand-link">
              ...
              <div>
                <div className="brand-title">{config.siteName ?? "技术生存服务器"}</div>
            ...
            <nav className="topbar-nav" aria-label="站内导航">
              ...
              <Link to="/rules" className="nav-text">
                服规
              </Link>
