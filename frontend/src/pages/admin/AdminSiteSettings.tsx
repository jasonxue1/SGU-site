...
import { THEME_PRESET_LABELS } from "../../lib/themePresets";
...
export function AdminSiteSettings() {
  ...
  function patchPaletteKey(key: "p1" | "p2" | "p3" | "p4", value: string) {
    setConfig((c) => ({
      ...c,
      customPalette: { ...c.customPalette, [key]: value },
    }));
  }
  ...
  return (
    <div>
      ...
      <form className="panel" onSubmit={(e) => void onSaveNow(e)}>
        ...
        <div className="row">
          <label>「加入我们」页面正文（Markdown）</label>
          <textarea value={config.joinPageBody ?? ""} onChange={(e) => patch("joinPageBody", e.target.value)} rows={8} />
        </div>
        <div className="row">
          <label>「服规」页面标题</label>
          <input value={config.groupRulesTitle ?? ""} onChange={(e) => patch("groupRulesTitle", e.target.value)} placeholder="默认：群规" />
        </div>
        <div className="row">
          <label>「服规」页面正文（Markdown）</label>
          <p className="muted" style={{ margin: "0 0 8px" }}>
            支持标题、列表、链接、代码块、表格（GFM）等。前台路由：<code>/rules</code>
          </p>
          <textarea value={config.groupRulesMarkdown ?? ""} onChange={(e) => patch("groupRulesMarkdown", e.target.value)} rows={14} />
        </div>
        <fieldset className="row" style={{ border: "1px solid var(--border)", borderRadius: 12, padding: "16px 18px" }}>
          <legend style={{ padding: "0 8px" }}>全站配色</legend>
          <p className="muted" style={{ marginTop: 0 }}>
            基于四档色板驱动背景、文字与强调色渐变。选择「自定义」后可填写 #RRGGBB（或使用取色器）。
          </p>
          <div className="theme-preset-grid">
            {(Object.keys(THEME_PRESET_LABELS) as Array<keyof typeof THEME_PRESET_LABELS>).map((id) => (
              <label key={id} className="theme-preset-option">
                <input
                  type="radio"
                  name="colorPreset"
                  checked={(config.colorPreset ?? "default") === id}
                  onChange={() => patch("colorPreset", id)}
                />
                {THEME_PRESET_LABELS[id]}
              </label>
            ))}
            <label className="theme-preset-option">
              <input
                type="radio"
                name="colorPreset"
                checked={(config.colorPreset ?? "default") === "custom"}
                onChange={() => patch("colorPreset", "custom")}
              />
              自定义
            </label>
          </div>
          {(config.colorPreset ?? "default") === "custom" ? (
            <div className="theme-custom-colors">
              {(
                ...
                <div className="row" key={key}>
                  ...
                  <input
                    type="text"
                    value={config.customPalette?.[key] ?? ""}
                    onChange={(e) => patchPaletteKey(key, e.target.value)}
                    placeholder="#RRGGBB"
                  />
                  <input
                    type="color"
                    value={hexForColorInput(config.customPalette?.[key])}
                    onChange={(e) => patchPaletteKey(key, e.target.value)}
