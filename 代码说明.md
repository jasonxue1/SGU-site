...
│   └── package.json
├── deploy/                  # Nginx 反代示例等（如 `nginx-reverse-proxy.example.conf`）
├── docs/
...
| `PORT` | 监听端口，默认 4000 |
| `LISTEN_HOST` | 可选；不设时生产环境默认 `0.0.0.0`，便于宝塔/Nginx 反代访问 |
...
| 宝塔面板部署 | `docs/宝塔面板部署.md` |
| 根目录一键安装+构建 | 仓库根目录 `npm run deploy:build` |
| PM2（固定 cwd、NODE_ENV） | `backend/ecosystem.config.cjs`、`npm run pm2:start` |
| Nginx 反代片段示例 | `deploy/nginx-reverse-proxy.example.conf` |
