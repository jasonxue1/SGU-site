...
publicRouter.get("/site", async (_req, res, next) => {
  try {
    ...
    let siteAnnouncements: AnnSelect[] = [];
    try {
      siteAnnouncements = await prisma.siteAnnouncement.findMany({
        where: { published: true },
        orderBy: { sortOrder: "asc" },
        select: {
          id: true,
      ...
      console.error(
        "[/api/public/site] 无法读取 site_announcements，请执行 prisma db push 并保证数据库已建表。",
        e,
      );
    }
    ...
    const serverHistoryOut = serverHistory.map((e) => ({
      ...e,
      imageUrl: normalizedMediaOrOriginal(e.imageUrl),
    }));
    const announcementsOut = siteAnnouncements.map((a) => ({
      ...a,
      linkUrl: a.linkUrl != null && isHttpOrHttpsUrl(a.linkUrl) ? a.linkUrl : null,
    }));
    res.json({
      config,
      cards: cardsOut,
      serverHistory: serverHistoryOut,
      members: membersOut,
      openSourceProjects: openSourceOut,
